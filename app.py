...
def index():
    ...
    sucesso = Vendedor.query.filter_by(status='Sucesso').count()
    vazio = Vendedor.query.filter_by(status='Vazio').count()
    erro = Vendedor.query.filter_by(status='Erro').count()
...
def import_vendedores():
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            vendedor = Vendedor.query.get(codigo)
            if not vendedor:
                vendedor = Vendedor(codigo=codigo)
...
def export_comissoes():
    comissoes = Comissao.query.order_by(Comissao.timestamp.desc()).all()
    data = []
    for c in comissoes:
        data.append({
            'Equipe': c.equipe,
...
def consultar():
    ...
    consulta_id = str(uuid.uuid4())[:8] # ID único para este lote
    ...
    for codigo in codigos:
        ...
        try:
            ...
            if not data or len(data) == 0:
                ...
                consultas_unicas = db.session.query(Comissao.consulta_id).filter_by(vendedor_codigo=codigo).distinct().all()
                if len(consultas_unicas) > 3:
                    # Remove a mais antiga
                    oldest_id = consultas_unicas[0][0]
                    Comissao.query.filter_by(vendedor_codigo=codigo, consulta_id=oldest_id).delete()
